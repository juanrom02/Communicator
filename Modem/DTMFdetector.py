...
class DTMFdetector(object):
	...
	def getDTMFfromRAW(self, filename):
		
		self.reset() #reset the current state of the detector
		
		audioFile = AudioSegment.from_file(filename, format="raw", frame_rate=8000, channels=1, sample_width=2)
		
		#print file.getparams()
		totalFrames = audioFile.frame_count()
		count = 0
		while totalFrames != count:
			raw = audioFile.get_frame(count)
